...
//
// This is NOT WINDOWS only stuff, its for non macos stuff
// Ptr crap was only done to please the mac apis
// Dont forget, unix exists , its not a windows/mac world.
//
#ifndef DEF_MAC
	#include <malloc.h>
	#define	NewPtr( data )		malloc(data)
	#define	DisposePtr( p )		free((Ptr)p)
	#ifndef		NULL
	#define 	NULL				0L
	#endif
	typedef 	void *			Ptr;
...
ICE_KEY *ice_key_create (int	n)
{
	...
	if ((ik = (ICE_KEY *) NewPtr (sizeof (ICE_KEY))) == NULL)
	    return (NULL);
	...
	if ((ik->ik_keysched = (ICE_SUBKEY *) NewPtr (ik->ik_rounds
...
void cycle_key( ICE_KEY	*ik )
{
	...
	for (i=0; i<ik->ik_rounds; i++)
	    for (j=0; j<3; j++){
			ik->ik_keysched[i][j] -= SUBVALUE;
			//ik->ik_keysched[i][j] ^= XORVALUE;
		}
}
...
unsigned char *ice_key_encrypt (const ICE_KEY *ik,const unsigned char *ptext,unsigned char	*ctext)
{
	register int		i;
...
unsigned char *ice_key_decrypt (const ICE_KEY *ik,const unsigned char	*ctext,unsigned char *ptext)
{
	register int			i;
	register unsigned long	l, r;
