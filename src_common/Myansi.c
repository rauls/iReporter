...
{
	DEF_ASSERT(nYear>=1970);
	DEF_ASSERT(nYear<=2035);
...
int RunSelf( const char *param )
{
	...
	sprintf( newparam, "application %s", param );
...
long mystrncpy( char *dst, const char *src, int max )	// c-string copy function
{
	...
	DEF_PRECONDITION(dst);   
	DEF_PRECONDITION(src);
	DEF_PRECONDITION(max>=0);	// crappy signed integers
...
{
	DEF_PRECONDITION(s1);	// don't remove!
	DEF_PRECONDITION(s2);	// ditto
