...
#define DIR_LIST_WIDTH   110
#define DIR_LIST_HEIGHT  180
#define FILE_LIST_WIDTH  110
...
static void gtk_file_dialog_class_init    (GtkFileSelectionClass *klass);
static void gtk_file_dialog_init          (GtkFileSelection      *filesel);
...
gtk_file_dialog_get_type (void)
{
  ...
  if (!file_selection_type)
	...
	(GtkClassInitFunc) gtk_file_dialog_class_init,
	(GtkObjectInitFunc) gtk_file_dialog_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
...
static void
gtk_file_dialog_class_init (GtkFileSelectionClass *class)
{
...
/********************************************
 ...
 Critical changes to dialog , this is all new
 stuff, the rest can be ignored since it is change
 to allow a non library change.

*********************************************/
#include "rename_icon.xpm"
#include "delete_icon.xpm"
#include "mkdir_icon.xpm"

char ok_button[32] = "Ok";

static void
gtk_file_dialog_init (GtkFileSelection *filesel)
{
  ...
  pulldown_hbox = gtk_hbox_new (TRUE, 3);
 ...
 // gtk_box_pack_start (GTK_BOX (filesel->main_vbox), filesel->button_area, FALSE, FALSE, 0);
  gtk_box_pack_start (GTK_BOX (pulldown_hbox), filesel->button_area, FALSE, FALSE, 0);
  gtk_widget_show (filesel->button_area);
  
  gtk_file_dialog_show_fileop_buttons(filesel);
  ...
  /*  The OK button  */
  filesel->ok_button = gtk_button_new_with_label (( ok_button ));
...
GtkWidget*
gtk_file_dialog_new (const gchar *title)
{
  GtkFileSelection *filesel;

  if ( strstr( title, "Save" ) )
	  strcpy( ok_button, "Save" );
  else
  if ( strstr( title, "Open" ) )
	  strcpy( ok_button, "Open" );

  filesel = gtk_type_new ( gtk_file_dialog_get_type() );
...
GtkWidget *gtk_file_add_buttonimage( GtkWindow *w, char *label_text , char *data_xpm)
{
	GdkPixmap *pixmap;
	GdkBitmap *mask;
	GdkColor	col;
	GtkWidget *button, *wpixmap, *box1, *label;
	
	button = gtk_button_new (); 
	pixmap = gdk_pixmap_create_from_xpm_d (w,
					 &mask, 
					 &col,
					 data_xpm);
	...
	gtk_box_pack_start (GTK_BOX (box1),wpixmap, FALSE, FALSE, 1);
...
void
gtk_file_dialog_show_fileop_buttons (GtkFileSelection *filesel)
{
  ...
  if (!filesel->fileop_c_dir) 
    {
      //filesel->fileop_c_dir = gtk_button_new_with_label (("Makedir"));
      filesel->fileop_c_dir = gtk_file_add_buttonimage (filesel->button_area->window, ("Makedir"), mkdir_icon );
  ...
  if (!filesel->fileop_del_file) 
    {
      filesel->fileop_del_file = gtk_file_add_buttonimage (filesel->button_area->window, ("Delete"), delete_icon);
  ...
  if (!filesel->fileop_ren_file)
    {
      filesel->fileop_ren_file = gtk_file_add_buttonimage (filesel->button_area->window, ("Rename"), rename_icon);
...
void       
gtk_file_dialog_hide_fileop_buttons (GtkFileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_FILE_SELECTION (filesel));
...
void
gtk_file_dialog_set_filename (GtkFileSelection *filesel,
...
gchar*
gtk_file_dialog_get_filename (GtkFileSelection *filesel)
{
