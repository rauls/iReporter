...
//extern "C" void ForceCheckSchedule( void );

void NTSLogSchedule( char *txt )
{
...
:CNTService("Report - Scheduler", 2, 0)
{
	m_iStartParam = 0;
	m_iIncParam = 1;
...
void CMyService::Run()
{
    ...
    while (m_bIsRunning) {
		...
		//DebugMsg("checking for schedule change");
		InitReadSchedule(2);

		if ( IsDemoTimedout() ){
		    DebugMsg("Demo timed out");
			m_bIsRunning = FALSE;
		} else
			ServiceCheckSchedule();
